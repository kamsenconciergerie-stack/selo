...
export function UserDashboard({ user }: UserDashboardProps) {
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      case 'confirmed': return 'bg-kamsen-blue-light text-green-800';
      case 'pending': return 'bg-yellow-100 text-yellow-800';
      case 'cancelled': return 'bg-red-100 text-red-800';
      case 'completed': return 'bg-kamsen-blue-light text-blue-800';
      default: return 'bg-kamsen-blue-light text-gray-800';
    }
  };

  const getPaymentStatusColor = (status: string) => {
    switch (status) {
      case 'completed': return 'bg-kamsen-blue-light text-green-800';
      case 'pending': return 'bg-kamsen-blue-light text-orange-800';
  ...
  return (
    <div className="max-w-6xl mx-auto p-6">
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-kamsen-blue">
            ...
            {bookings.length === 0 ? (
              <Card>
                <CardContent className="p-6 text-center">
                  <Package className="mx-auto h-12 w-12 text-kamsen-gray mb-4" />
              ...
              bookings.map((booking: any) => (
                <Card key={booking.id}>
                  ...
                  <CardContent>
                    ...
                    {booking.notes && (
                      <div className="mt-4 p-3 bg-kamsen-blue-light rounded-md">
                        <p className="text-sm text-kamsen-blue">{booking.notes}</p>
                      </div>
                    )}
                  </CardContent>
                </Card>
        ...
        <TabsContent value="reviews" className="space-y-4">
          <div className="grid gap-4">
            {reviews.length === 0 ? (
              <Card>
                <CardContent className="p-6 text-center">
                  <Star className="mx-auto h-12 w-12 text-kamsen-gray mb-4" />
                  <h3 className="text-lg font-medium text-kamsen-blue mb-2">
                    Aucun avis donné
                  </h3>
                  <p className="text-kamsen-gray">
              ...
              reviews.map((review: any) => (
                <Card key={review.id}>
                  ...
                  {review.comment && (
                    <CardContent>
                      <p className="text-kamsen-blue">{review.comment}</p>
                    </CardContent>
                  )}
                </Card>
              ))
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className="text-sm font-medium text-kamsen-blue">Prénom</label>
                  <div className="mt-1 p-2 bg-kamsen-blue-light rounded-md">
                    {user.firstName}
                  </div>
                </div>
                <div>
                  <label className="text-sm font-medium text-kamsen-blue">Nom</label>
                  <div className="mt-1 p-2 bg-kamsen-blue-light rounded-md">
                    {user.lastName}
                  </div>
                </div>
                <div>
                  <label className="text-sm font-medium text-kamsen-blue">Email</label>
                  <div className="mt-1 p-2 bg-kamsen-blue-light rounded-md">
                    {user.email}
                  </div>
                </div>
                <div>
                  <label className="text-sm font-medium text-kamsen-blue">Rôle</label>
                  <div className="mt-1 p-2 bg-kamsen-blue-light rounded-md">
